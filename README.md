...
## Virtual machine
RVM currently supports `37` instructions, listed below:
...
Floating-point instructions operate on the raw 64-bit register contents
reinterpreted as IEEE 754 doubles (`LA`/`SA` move the bits unchanged), so
they require 64-bit `size_t`. Semantics:

- `FADD`/`FSUB`/`FMUL`/`FDIV` follow IEEE 754: `FDIV` by zero yields
  `+/-inf` and never halts (unlike integer `DIV`).
- `FCMP`/`FLT`/`FLE` set the destination register to `1` or `0` for use
  with the regular `JNZ`/`JZ` jumps; `NaN` compares false everywhere.
- `FTOI` truncates toward zero and halts with an error on `NaN` or values
  outside `[0, 2^64)` (out-of-range float-to-int conversion is undefined
  in C).
- `FLD` loads a double constant (e.g. `FLD R0 3.14`); `FPRT` prints a
  register with full round-trip precision (`%.17g`).
...
`fib(30)` benchmark in both integer (`test/test.rvs`) and floating-point
(`test/fib_float.rvs`) form (expected output `832040`), plus the
floating-point smoke test (`test/float_test.rvs`).
