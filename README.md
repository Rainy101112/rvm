...
`test/test.rvs` is generated by `test/gen_fib.py` (recursion via an explicit
stack and self-modifying code, since RVM has no CALL/RET and no register
indirect addressing). Regenerate with `FIB_N=<n> python3 test/gen_fib.py`.
